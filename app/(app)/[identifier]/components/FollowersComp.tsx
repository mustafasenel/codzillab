...
const FollowersComp: React.FC<FollowersProps> = ({ currentUser, userId }) => {
  ...
  const fetchPosts = async ({
    ...
    const response = await fetch(
      `/api/users/getFollowersById?skip=${pageParam}&take=${take}&userId=${userId}&search=${searchQuery}`
    );
    if (!response.ok) throw new Error("Network response was not ok");
    const data = await response.json();
    if (!Array.isArray(data.followers)) return [];
  ...
  const {
    ...
    queryKey: ["followers", searchQuery, userId],
    queryFn: fetchPosts,
