...
const FollowingsComp: React.FC<FollowersProps> = ({ currentUser, userId }) => {
  ...
  const fetchPosts = async ({
    ...
    const response = await fetch(
      `/api/users/getFollowingsById?skip=${pageParam}&take=${take}&userId=${userId}&search=${searchQuery}`
    );
    if (!response.ok) throw new Error("Network response was not ok");
    const data = await response.json();
    if (!Array.isArray(data.followings)) return [];
  ...
  const {
    ...
    queryKey: ["followings", searchQuery, userId],
    queryFn: fetchPosts,
