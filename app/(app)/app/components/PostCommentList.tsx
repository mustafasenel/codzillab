...
const fetchComments = async ({
    ...
    const response = await fetch(`/api/post/comment/getComments?postId=${postId}&skip=${pageParam}&take=${take}`);
...
const PostCommentList: React.FC<{ postId: string; currentUser: User }> = ({
    postId,
    currentUser,
}) => {
    const {
            ...
            {data?.pages.flatMap((group) => group).length === 0 ? (
                ...
                data?.pages.flatMap((group) =>
                    group.map((comment: FullCommentType) => (
                        <PostCommentItem key={comment.id} comment={comment} currentUser={currentUser} />
                    ))
                )
            )}
            {hasNextPage && (
