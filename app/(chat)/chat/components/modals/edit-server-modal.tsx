...
const EditServerModal = () => {
  ...
  useEffect(() => {
    const initializeFile = async () => {
      if (server?.imageUrl) {
        const file = await urlToFile(server.imageUrl, "uploaded-image.jpg", "image/jpeg");
        setFile(file); // File olarak set edilir
        form.setValue("imageUrl", server.imageUrl); // URL değeri formda kalır
      }
    };
    initializeFile();
  }, [server, form]);
    ...
    <Dialog open={isModalOpen} onOpenChange={handleClose}>
      <DialogContent className="p-0 overflow-hidden">
        <DialogHeader className="pt-8 px-6">
          ...
          <DialogDescription className="text-center text-zinc-500 dark:text-muted-foreground">
