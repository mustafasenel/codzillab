...
type InputProps = {
  ...
  onDelete?: (fileUrl: string) => Promise<void>; // URL string olarak
...
const MultiImageDropzone = React.forwardRef<HTMLInputElement, InputProps>(
  (
    { dropzoneOptions, value, className, disabled, onChange, onFilesAdded, onDelete },
    ref
  ) => {
    ...
    return (
      <div className="relative w-full">
        {disabled && (
          <div className="flex items-center justify-center absolute inset-y-0 h-full w-full bg-background/80">
            <ClipLoader size={20} className="" />
          </div>
        )}
        <div className="flex flex-wrap gap-2">
  {/* Images */}
  {value?.map(({ file, progress }, index) => (
    <div key={index} className={`${variants.image} aspect-square flex-1 min-w-[calc(80%)] sm:min-w-[calc(40%)] lg:min-w-[calc(30%)] xl:min-w-[calc(22%)] max-w-60`}>
      <img 
      ...
      {imageUrls[index] && !disabled && (
        <div
          ...
          onClick={async (e) => {
            ...
            const fileUrlToRemove = value[index]?.url || ""; // publicUrl'yi kullan
      
            // Backend'e silme işlemi
            await onDelete?.(fileUrlToRemove!);
            console.log(fileUrlToRemove)
            // Görüntü listesinden sil
  ...
  {(!value || value.length < (dropzoneOptions?.maxFiles ?? 0)) && (
    <div
      {...getRootProps({
        className: `flex ${value?.length === 0 ? 'w-full h-40' : ''} ${dropZoneClassName}`,
      })}
    >
      {/* Main File Input */}
